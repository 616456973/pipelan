...
**筛选条** (5 个):
- 团队 / 客户 / 负责人 (新) / 阶段 / 币种 / 发票状态 (新) / 搜索框
- ~~产品 / 产品线~~ (移除 — 直接在表格列里看)

**表格列** (10):
- # / 团队 / 负责人 (新) / 商机 / 客户 (新) / 产品 / 阶段 / 发票状态 (新) / 含税 / RMB / 赢率 / 操作
...
用户已有的 DB 数据 (从 v2.0 导入) 缺 4 个新字段 (owner/customer/sales_channel/invoice_status/amount_rmb_equivalent). 启动时检测:
